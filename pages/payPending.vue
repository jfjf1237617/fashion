<template>
  <div>
    <div class="pay-padding-container">
      <div ref="shipped" class="shipped-container">
        <p class="title">Pay Pending</p>
      </div>
          ...
          <el-table-column class="await-ship-table-item" label="Merchant Pay">
            <template slot-scope="scope">
              <el-button
                ...
                class="shipped-checkout"
                >{{ payStatus(scope.row.payStatus) }}</el-button
              >
            </template>
          </el-table-column>
        </el-table>
  ...
  async fetch({ app, store }) {
    ...
    if (process.server) {
      let res = await app.$axios.get(
        `/shopify/orders?fulfillmentStatus=unshipped&payPadding=1&limit=10`
      );
  ...
  methods: {
    ...
    async toCheckout(productDetail: any) {
      if (productDetail.payStatus > 0 || productDetail.payStatus === -100) {
        ...
        try {
          let res = await shopifyService.postShopifyOrderPay(productDetail.id);
